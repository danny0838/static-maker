...
const ori_expr = document.getElementById('ori-expr'), filter_expr = document.getElementById("filter-expr"), sort_expr = document.getElementById("sort-expr"), search_result = document.getElementById("search-result"), tbody = document.getElementById("tbody"), pages_a = document.getElementById("pages-a");

var hide_seach = !1, last_forms, form_s = 5;

var per_page = 10;

const tables = document.getElementById("tables"), toggle_s = document.getElementById("toggle-s"), only_my_fav = document.getElementById("only-my-fav"), def_lv_e = document.getElementById("def-lv"), plus_lv_e = document.getElementById("plus-lv"), cattype_e = document.getElementById("cattype"), trait_s = document.getElementById("trait-s"), atk_s = document.getElementById("atk-s"), ab_s = document.getElementById("ab-s"), atkBtn = atk_s.firstElementChild.firstElementChild, traitBtn = trait_s.firstElementChild.firstElementChild, abBtn = ab_s.firstElementChild.firstElementChild, name_search = document.getElementById("name-search");
...
function calculate(code = "") {
    pages_a.textContent = "";
    const sortCode = simplify(sort_expr.value), url = (def_lv = Math.min(Math.max(parseInt(def_lv_e.value), 1), 60), 
    ...
    if (code.length) {
        ...
        const codes = [], cattypes = Array.from(cattype_e.querySelectorAll(".o-selected"));
        if (cattypes.length) {
...
loadAllCats().then(_cats => {
    ...
    for (C of s) for (let f of C.forms) (f.name.includes(search) || f.jp_name.includes(search)) && results.push([ 1, f ]);
    renderTable(results);
};

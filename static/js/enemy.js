...
function calc() {
	...
	if (E.trait & TB_ALIEN) {
		...
		let m;
		switch (E.star) {
		case 2: m = 11 - (treasures[20] / 10);break;
		case 3: m = 11 - (treasures[24] / 10);break;
		case 4: m = 11 - (treasures[30] / 10);break;
		}
		HP *= m;
		atk *= m;
		atk1 *= m;
...
function search_for() {
	...
	req.onsuccess = function(e) {
		...
		db.transaction("map").objectStore("map").get(-1).onsuccess = function(e) {
			...
			if (char_groups && char_groups['ver'] == {{stage}})
				really_search();
			else
				load_all();
		}
...
.then(e => {
	...
	X = new URL('https://battlecats-db.com/enemy/' + t3str(my_id) + '.html');
